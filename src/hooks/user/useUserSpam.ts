 ...
 */
export const useUserSpam = (api: WildDuckAPI, userId?: string) => {
  const queryClient = useQueryClient();

  // Query to get user spam settings
  const spamQuery = useQuery({
    queryKey: ["user", userId, "spam"],
    queryFn: async (): Promise<SpamSettings | undefined> => {
  ...
  const updateSpam = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["user", variables.userId, "spam"],
      });
    },
  });
  ...
  const updateSpamLevel = useMutation({
    mutationFn: async ({
      ...
      return await api.updateUser(userId, { spamLevel });
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ["user", variables.userId] });
      queryClient.invalidateQueries({
  ...
  const addToWhitelist = useMutation({
    mutationFn: async ({
      ...
      return await api.updateUser(userId, { fromWhitelist: newWhitelist });
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ["user", variables.userId] });
      queryClient.invalidateQueries({
  ...
  const clearWhitelist = useMutation({
    mutationFn: async (userId: string) => {
      return await api.updateUser(userId, { fromWhitelist: [] });
    },
    onSuccess: (_, userId) => {
      queryClient.invalidateQueries({ queryKey: ["user", userId] });
      queryClient.invalidateQueries({ queryKey: ["user", userId, "spam"] });
    },
  });
